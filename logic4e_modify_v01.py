...
#propositions by rminalr

#wumpus_kb.tell(~B11)
#wumpus_kb.tell(B21)

#print(to_cnf('~(P11 <=> P12)'))
...
########
#Start from Here
#modifications from leonardo bobadilla



'''
#testing Hybrid humpus


hybrid_wumpus = HybridWumpusAgent(4)
print("Testing Hybrid Wumpus")

print(hybrid_wumpus)
print(hybrid_wumpus.kb)

print(hybrid_wumpus.kb.ask_if_true(wumpus_alive(0)))
print(hybrid_wumpus.kb.ask_if_true((have_arrow(0))))
print(hybrid_wumpus.kb.ask_if_true((facing_east(0))))
#print(hybrid_wumpus.kb.ask_if_true((facing_west(0))))


hybrid_wumpus.execute(Stench)
...
#print( "Privacy preserving Wumpus")

P11, P12, P21, P22, P31, B11, B21 = expr('P11, P12, P21, P22, P31, B11, B21')


#Public prositions \alpha_public

wumpus_kb.tell(~P11)
wumpus_kb.tell(B11 | '<=>' | (P12 | P21))
wumpus_kb.tell(B21 | '<=>' | (P11 | P22 | P31))

#propositions by \alpha_1

wumpus_kb.tell(~B11)

#propositions by \alpha_2

wumpus_kb.tell(B21)

#propositions by \beta

print(wumpus_kb.ask(~B11))


#TODO: figure out how ask works
...
#(B21 | '<=>' | (P11 | P22 | P31))
s_3 = to_cnf('(B21 <=> ((P11 | P22 | P31)))')
print(s_3)

alpha_public = to_cnf('(~P11 & (B11 <=> (P12 | P21)) & (B21 <=> ((P11 | P22 | P31)))    )')
print(alpha_public)  
...
#I just need to change dpll_

alpha_combined = to_cnf('(~P11 & (B11 <=> (P12 | P21)) & (B21 <=> ((P11 | P22 | P31))) & ~B11 &  B21 & P12)')
#I just need to change to 
result = dpll_satisfiable(alpha_combined)
print(result)
...
#print(s)
#TODO:
#1) Integrate it with the ppSAT
#2) Look at 7.7 see we can use this in to a hybrid agent 



print("First Order Logic Example 1")


kb0 = FolKB([expr('Farmer(Mac)'), expr('Rabbit(Pete)'), expr('(Rabbit(r) & Farmer(f)) ==> Hates(f, r)')])
kb0.tell(expr('Rabbit(Flopsie)'))
#kb0.retract(expr('Rabbit(Pete)'))
print(kb0.ask(expr('Hates(Mac, x)'))[x])
#kb0.ask(expr('Wife(Pete, x)'))


print("First Order Logic Example 2")
...
print(crime_kb.ask(expr('Criminal(West)')))

print(crime_kb.ask(expr('Criminal(x)'))[x])



print("Print all the clauses")

for element in crime_kb.clauses:
    print(element)

kb_consts = list({c for clause in crime_kb.clauses for c in constant_symbols(clause)})
#query_vars = list({v for clause in p for v in variables(clause)})
...
family_kb = FolKB(
    map(expr, ['(King(x) & Greedy(x)) ==> Evil(x)',
               'King(John)',
               'Greedy(John)',
               'Brother(Richard, John)']))


print(family_kb.ask(expr('Evil(x)'))[x])
...
#Add sentences that do not have variables, still hard coded

for element in family_kb.clauses:
...
kb_consts = list({c for clause in family_kb.clauses for c in constant_symbols(clause)})
print(kb_consts)


#substituting ground terms in clasuse with variables, this is hard coded still

for c in kb_consts:
    
    symbol = str(c)
    txt = str(family_kb.clauses[0])
    print (c)
    print (family_kb.clauses[0])

    x = txt.replace("x", symbol)
...
#TEST
#alpha_combined = to_cnf('King(John) & Greedy(John) & Brother(Richard, John) & ( (King(Richard) & Greedy(Richard)) ==> Evil(Richard))')
#print(alpha_combined)
#result = dpll_satisfiable(alpha_combined)
#print(result)


#['King(John)', 'Greedy(John)', 'Brother(Richard, John)', '((King(Richard) & Greedy(Richard)) ==> Evil(Richard))', '((King(John) & Greedy(John)) ==> Evil(John))'])
#alpha_combined = to_cnf('(~P11 & (B11 <=> (P12 | P21)) & (B21 <=> ((P11 | P22 | P31))) & ~B11 &  B21 & P12)')
...
result2 = " & ".join(expression_list)

print(result2)


alpha_combined2 = to_cnf(result2)

result3 = dpll_satisfiable(alpha_combined2)

print(result3)


#See if we can infer Evil(John)

result4 = result2 +" & "+ "~Evil(John)"
print(result4)

alpha_combined3 = to_cnf(result4)


result5 = dpll_satisfiable(alpha_combined3)
